  ...
  async run(client, message, args) {
    ...
    if (!args[0])
      return message.channel.send(
        "You need to say your choice. rock, paper, or scissors?"
      );
    if (!["rock", "paper", "scissors"].includes(args[0]))
      return message.channel.send(
        "You did not choose one of the options: `rock`, `paper`, or `scissors`."
      );
    ...
    if (botChoiceStr == args[0])
      return message.channel.send(
        `${botEmoji} vs ${playerEmoji}! We tied! Try again..`
      );
    if (args[0] == "rock") {
      if (botChoiceStr == "paper")
        return message.channel.send(
          `${botEmoji} vs ${playerEmoji}! You lost.. Better luck next time!`
        );
      else
        return message.channel.send(
          `${botEmoji} vs ${playerEmoji}! You won! Good job!`
        );
    }
    if (args[0] == "paper") {
