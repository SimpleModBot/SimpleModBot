...
module.exports = {
    ...
    async execute(message, args, data, client) {
        ...
        if (memberRole) {
            ...
            // const captchaAttachment = new MessageAttachment(await captcha.image, 'captcha.png');
            // const msg = await message.reply({ embeds: [new Discord.MessageEmbed().setDescription(`Complete the captcha to verify! ${captcha.text}`).setImage('attachment://captcha.png').setColor('GREY')], files: [captchaAttachment], allowedMentions: { repliedUser: false } });
            ...
            //     if (response) {
                    ...
                    message.channel.send({ embeds: [new Discord.MessageEmbed().setDescription("You have successfully been verified in the server!").setColor('GREY')] });
                // };
            } catch (err) {
