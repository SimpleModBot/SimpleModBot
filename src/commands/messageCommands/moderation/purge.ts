...
module.exports = {
    ...
    async execute(message, args, data, client) {
        ...
        if (args[0] == "ENA") return message.channel.send({ embeds: [new Discord.MessageEmbed().setDescription("You must state a number of messages to delete.").setColor('GREY')] });
        ...
        if (!amountToDelete || amountToDelete < 2 || amountToDelete > 200) return message.channel.send({ embeds: [new Discord.MessageEmbed().setDescription("The number stated must be between 2 and 200.").setColor('GREY')] });
        const fetched = await message.channel.messages.fetch({ limit: amountToDelete + 1, });
