...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(o => o
            .setName("font")
            .setDescription("the font")
            .setChoices(
                { name: "3D", value: "3-D" },
                { name: "3D ASCII", value: "3D-ASCII" },
                { name: "Big Money North East", value: "Big Money-ne" },
    ...
    async exec(interaction: ChatInputCommandInteraction) {
        ...
        if(txt.length > 2000) {
            await interaction.reply({ content: "The message is too long. try again with a shorter message!" });
        } else {
