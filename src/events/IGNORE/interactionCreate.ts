...
module.exports = {
	...
	async execute(interaction, client) {
		...
		if (interaction.isCommand()) {
			...
			if (interaction.devOnly == true && !client.devIDs.includes(interaction.member.id))
				return interaction.reply({ embeds: [new Discord.MessageEmbed().setDescription(`You aren't a developer and therefor, don't have permission to do this action!`).setColor('GREY')], ephemeral: true });
		...
		if (interaction.isButton()) {
			if (interaction.customId == 'tic') {
				...
				const del = new MessageActionRow().addComponents(new MessageButton().setCustomId('del').setLabel('Delete Ticket!').setStyle('DANGER'));
