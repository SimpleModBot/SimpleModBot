...
module.exports = {
    ...
    async execute(message, args, data, client) {
        ...
        channel.send({ embeds: [new Discord.MessageEmbed().setDescription(args.join(' ')).setFooter({ text: `${message.author.tag} - ${message.author.id}`, iconURL: message.author.displayAvatarURL() }).setColor('GREY')] });
