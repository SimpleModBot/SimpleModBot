...
module.exports = {
	...
	async execute(member, client) {
		...
		const welcomeEmbed = new Discord.MessageEmbed().setTitle('A new user has joined!').setDescription(desc).setTimestamp().setColor('GREY');

		if (welcomechannel.permissionsFor(member.guild.me).has(['SEND_MESSAGES']))
			welcomechannel.send({
				content: 'Welcome!',
				embeds: [welcomeEmbed],
			});
